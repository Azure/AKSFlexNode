...
func TestDetectAndRemediate_SkipsStaleSpec_DoesNotCallDetectors(t *testing.T) {
	...
	err := detectAndRemediate(context.Background(), nil, logger, nil, []Detector{d}, specSnap, statusSnap, nil, "kube1")
	if err != nil {
		t.Fatalf("err=%v, want nil", err)
	}
...
func TestDetectAndRemediate_BootstrapGuard_SkipsWhenInProgress(t *testing.T) {
	...
	err := detectAndRemediate(context.Background(), nil, logger, &bootstrapInProgress, []Detector{d}, specSnap, statusSnap, nil, "kube1")
	if err != nil {
		t.Fatalf("err=%v, want nil", err)
	}
