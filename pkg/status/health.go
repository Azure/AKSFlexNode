...
func MarkKubeletUnhealthyBestEffort(logger *slog.Logger) {
...
func MarkKubeletUnhealthyBestEffortAtPath(logger *slog.Logger, statusFilePath string, now time.Time) {
	if statusFilePath == "" {
		return
	}
	if now.IsZero() {
	...
	_ = withStatusFileLock(func() error {
		...
		if err := writeStatusToFileUnlocked(statusFilePath, snap); err != nil {
			logger.Debug("failed to mark status unhealthy", "path", statusFilePath, "error", err)
		}
		return nil
	})
}
...
func MarkKubeletHealthyAfterUpgradeBestEffort(logger *slog.Logger, kubeletVersion string) {
	statusFilePath := spec.StatusFilePath
...
//
// If now is zero, time.Now() is used.
func MarkKubeletHealthyAfterUpgradeBestEffortAtPath(logger *slog.Logger, statusFilePath string, kubeletVersion string, now time.Time) {
	if statusFilePath == "" {
		return
	}
	if now.IsZero() {
	...
	_ = withStatusFileLock(func() error {
		...
		if err := writeStatusToFileUnlocked(statusFilePath, snap); err != nil {
			logger.Debug("failed to mark status healthy after upgrade", "path", statusFilePath, "error", err)
		}
		return nil
	})
}
