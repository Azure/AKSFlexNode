...
func NewAzureClient(cred azcore.TokenCredential, subscriptionID string, logger *Logger) (*AzureClient, error) {
	...
	if c.diskClient, err = armcompute.NewDisksClient(subscriptionID, cred, nil); err != nil {
		return nil, fmt.Errorf("failed to create disk client: %w", err)
	}
	if c.vmssClient, err = armcompute.NewVirtualMachineScaleSetsClient(subscriptionID, cred, nil); err != nil {
	...
	if c.arcClient, err = armhybridcompute.NewMachinesClient(subscriptionID, cred, nil); err != nil {
		return nil, fmt.Errorf("failed to create Arc client: %w", err)
	}
...
func (c *AzureClient) GetVNetInfo(ctx context.Context, nodeResourceGroup string) (vnetName, vnetRG string, err error) {
	...
	for pager.More() {
		...
		for _, vmss := range page.Value {
			subnetID := extractSubnetIDFromVMSS(vmss)
			if subnetID == "" {
				continue
			}
...
// DeleteNIC deletes a network interface if it exists.
func (c *AzureClient) DeleteNIC(ctx context.Context, resourceGroup, nicName string) error {
	poller, err := c.nicClient.BeginDelete(ctx, resourceGroup, nicName, nil)
	if err != nil {
		if isNotFoundError(err) {
			return nil
		}
...
func (c *AzureClient) DeleteDisks(ctx context.Context, resourceGroup, pattern string) error {
	pager := c.diskClient.NewListByResourceGroupPager(resourceGroup, nil)
	for pager.More() {
		page, err := pager.NextPage(ctx)
		if err != nil {
			return nil // Ignore errors
		...
		for _, disk := range page.Value {
			if disk.Name == nil || !strings.Contains(*disk.Name, pattern) {
				continue
			}
			poller, err := c.diskClient.BeginDelete(ctx, resourceGroup, *disk.Name, nil)
			if err != nil {
				continue
			}
			_, _ = poller.PollUntilDone(ctx, nil)
...
// extractSubnetIDFromVMSS extracts the subnet ID from a VMSS's network profile.
func extractSubnetIDFromVMSS(vmss *armcompute.VirtualMachineScaleSet) string {
	if vmss.Properties == nil ||
		vmss.Properties.VirtualMachineProfile == nil ||
		vmss.Properties.VirtualMachineProfile.NetworkProfile == nil {
		return ""
	}
	configs := vmss.Properties.VirtualMachineProfile.NetworkProfile.NetworkInterfaceConfigurations
	if len(configs) == 0 || configs[0].Properties == nil {
		return ""
	}
	ipConfigs := configs[0].Properties.IPConfigurations
	if len(ipConfigs) == 0 || ipConfigs[0].Properties == nil || ipConfigs[0].Properties.Subnet == nil || ipConfigs[0].Properties.Subnet.ID == nil {
		return ""
	}
	return *ipConfigs[0].Properties.Subnet.ID
}
