...
#   validate_node_not_ready <vm_name> - Wait for a node to become NotReady
#   validate_all_nodes_absent         - Verify kubeadm node is NotReady after reset
...
# ---------------------------------------------------------------------------
# validate_node_not_ready - Wait for a node to become NotReady
# ---------------------------------------------------------------------------
validate_node_not_ready() {
  local vm_name="$1"
  local timeout="${E2E_NODE_JOIN_TIMEOUT}"
  local elapsed=0

  log_info "Waiting for node '${vm_name}' to become NotReady (timeout: ${timeout}s)..."

  while [[ "${elapsed}" -lt "${timeout}" ]]; do
    local status
    status="$(kubectl get node "${vm_name}" -o jsonpath='{.status.conditions[?(@.type=="Ready")].status}' 2>/dev/null || echo "Unknown")"
    if [[ "${status}" != "True" ]]; then
      log_success "Node '${vm_name}' is NotReady (status: ${status})"
      return 0
    fi
    sleep 10
    elapsed=$((elapsed + 10))
  ...
  log_error "Node '${vm_name}' still Ready after ${timeout}s"
  log_error "Current nodes:"
...
# ---------------------------------------------------------------------------
# validate_all_nodes_absent - Validate kubeadm node is NotReady after reset
# ---------------------------------------------------------------------------
validate_all_nodes_absent() {
  log_section "Validating Kubeadm Node NotReady After Reset"

  local kubeadm_vm_name
  kubeadm_vm_name="$(state_get kubeadm_vm_name)"

  # Only kubeadm unjoin is implemented today. After reset the node object
  # remains in the API server but kubelet is stopped, so it should become
  # NotReady.
  local failed=0
  validate_node_not_ready "${kubeadm_vm_name}" || failed=1

  if [[ "${failed}" -eq 1 ]]; then
    log_error "Kubeadm node still Ready after reset"
    return 1
  fi
  ...
  log_info "All cluster nodes:"
  kubectl get nodes -o wide
  log_success "Kubeadm node confirmed NotReady after reset"
}
