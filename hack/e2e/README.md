...
| `upgrade-drift` | Join the token node with an older kubelet version and validate drift repave to the alternate nspawn side |
...
| `E2E_DRIFT_INITIAL_KUBERNETES_VERSION` | previous AKS minor | Initial token-node kubelet version for `upgrade-drift` |
