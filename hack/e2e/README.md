...
The `e2e-testing-external` environment should require maintainer review and disable administrator bypass where practical. Self-review remains allowed because a maintainer action such as rebasing a Dependabot PR becomes the workflow run's triggering actor; preventing self-review could otherwise leave the run with no eligible approver. The Azure identity should be limited to isolated, quota-constrained E2E resources. This repository uses customized OIDC subjects in the form `repository_owner_id:<OWNER_ID>:repository_id:<REPOSITORY_ID>:environment:<ENVIRONMENT>`. Configure Azure federated credentials for both `e2e-testing` and `e2e-testing-external` using the subject emitted by GitHub's OIDC login diagnostics rather than committing the numeric IDs. Where supported, also constrain the Dependabot OIDC `event_name` claim appropriately.
