...
For all three Azure identity modes, assign **Azure Kubernetes Service Flex Node
Agent Role** at
...
those settings: the role authorizes ARM operations; Kubernetes access uses
separate token/CSR credentials.
...
| `agent.hostPrefix` | string | Optional prefix for the agent's host-side binaries and helpers, under `<prefix>/bin`, `<prefix>/lib/aks-flex-node`, and `<prefix>/libexec`. It does not move the config, state, logs, systemd units, or anything inside the nspawn machine. Hosts with a read-only `/usr`, such as Azure Container Linux, must set a writable location; `aks-flex-node ignition` uses `/opt/aks-flex-node`. Must be an absolute, normalized path other than `/`. Run `aks-flex-node reset` before changing it, since reset only cleans the configured prefix and the default. Defaults to `/usr/local`. | `/opt/aks-flex-node` |
