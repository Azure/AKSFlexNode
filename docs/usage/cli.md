...
| `aks-flex-node ignition [flags] -- BOOTSTRAP_ARGS...` | Render an Ignition config that bootstraps the host on first boot | — | listed | `--base-config`; `--host-prefix`; `--output`, `-o` (default: `-`); `--sp-client-certificate-file`; `--sp-client-secret-file` |
