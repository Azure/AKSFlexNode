...
**Rollout prerequisite:** the role is currently only partially published
(canary environments); it must be published and visible in the target
environment before onboarding. If the check below fails, stop and resolve
publication or operator access. Do not substitute Contributor or an admin role.
...
FLEX_HOST_PRINCIPAL_ID="<host-identity-service-principal-object-id>"
