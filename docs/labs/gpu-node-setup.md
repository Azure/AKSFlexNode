...
>
> **Last validation attempt:** 2026-09-20; skipped because the available H100 VM family quota was `0` in the validation subscription
>
...
> [!IMPORTANT]
> For an Azure GPU VM, enable a managed identity, authorize it at the AKS cluster scope, and ensure the Flex node pool exists by following the [identity-backed operator workflow](../usage/getting-started.md). For a host outside Azure, use its Azure Arc or service principal path.
...
# For the Azure VM managed-identity path:
