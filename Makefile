...
.PHONY: docs-cli-generate
docs-cli-generate:
	@echo "Generating CLI reference..."
	@UPDATE_CLI_DOCS=1 go test ./cmd/aks-flex-node -run '^TestCLIReferenceIsCurrent$$' -count=1

.PHONY: docs-cli-check
docs-cli-check:
	@echo "Checking CLI reference..."
	@go test ./cmd/aks-flex-node -run '^TestCLIReferenceIsCurrent$$' -count=1

.PHONY: docs-freshness
docs-freshness:
	@echo "Checking lab validation freshness..."
	@python3 hack/docs/check_lab_freshness.py

.PHONY: docs-check
docs-check: docs-cli-check
	@echo "Checking documentation URL contract..."
	@python3 hack/docs/check_url_contract.py
	@echo "Checking documentation content..."
	@python3 hack/docs/check_docs.py
...
help:
	...
	@echo "  vet                Run go vet"
	@echo "  docs-cli-generate  Regenerate the CLI command and flag reference"
	@echo "  docs-cli-check     Check the CLI reference against Cobra commands"
	@echo "  docs-freshness     Report stale lab validation dates"
	@echo "  docs-check         Check CLI docs, links, safety rules, paths, and anchors"
