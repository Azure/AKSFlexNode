...
func renderStaticRoutesScript(routes []*linux.StaticRoute) (string, error) {
	...
	b.WriteString("DEFAULT_DEV=\"\"\n")
	b.WriteString("resolve_default_dev_cached() {\n")
	b.WriteString("  if [ -n \"$DEFAULT_DEV\" ]; then echo \"$DEFAULT_DEV\"; return 0; fi\n")
	b.WriteString("  DEFAULT_DEV=$(resolve_default_dev) || return 1\n")
	b.WriteString("  echo \"$DEFAULT_DEV\"\n")
	b.WriteString("}\n\n")

	b.WriteString("while IFS='|' read -r DEST DEV GW METRIC; do\n")
	b.WriteString("  [ -z \"$DEST\" ] && continue\n")
	fmt.Fprintf(&b, "  if [ \"$DEV\" = %q ]; then\n", autoDevToken)
	b.WriteString("    DEV=$(resolve_default_dev_cached) || { echo \"no default IPv4 route; cannot install route $DEST\" >&2; exit 1; }\n")
	b.WriteString("  fi\n")
	fmt.Fprintf(&b, "  if [ \"$GW\" = %q ]; then\n", autoGWToken)
	...
	b.WriteString("    GW=$(resolve_default_gw \"$DEV\") || { echo \"no default gateway on $DEV after 30s; cannot install route $DEST\" >&2; exit 1; }\n")
	b.WriteString("  fi\n")
	b.WriteString("  if [ \"$METRIC\" -gt 0 ]; then\n")
	b.WriteString("    ip -4 route replace \"$DEST\" via \"$GW\" dev \"$DEV\" metric \"$METRIC\"\n")
	b.WriteString("  else\n")
	b.WriteString("    ip -4 route replace \"$DEST\" via \"$GW\" dev \"$DEV\"\n")
	b.WriteString("  fi\n")
	b.WriteString("done <<'EOF'\n")
	for _, e := range entries {
		fmt.Fprintf(&b, "%s|%s|%s|%d\n", e.dest, e.dev, e.gw, e.metric)
	}
	b.WriteString("EOF\n")
	b.WriteString("exit 0\n")
	return b.String(), nil
