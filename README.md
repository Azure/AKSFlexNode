...
After the cluster, Unbounded-Net Sites, Flex node pool, private Layer 3 path, and host managed identity are ready, the host attachment is a short operation. The identity must have **Azure Kubernetes Service Contributor Role** at the AKS cluster scope. This example uses a system-assigned managed identity; use the [joining guide](docs/usage/joining-nodes.md) for Azure Arc or service principal authentication.
...
If the Node becomes `Ready` while the agent service restarts, inspect and approve the daemon CSR by following the [operator guide](docs/usage/getting-started.md#7-approve-the-daemon-csr-when-required).
